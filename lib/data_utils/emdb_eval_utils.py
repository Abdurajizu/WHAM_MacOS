...
def preprocess(dset, batch_size):
    ...
    for sequence in emdb_sequence_roots:
        ...
        for i, patch in enumerate(patch_list):
            bbox = bboxes[i*batch_size:min((i+1)*batch_size, len(frame_ids))].float().to(device)
            bbox_center = bbox[:, :2]
            bbox_scale = bbox[:, 2] / 200

            feature = extractor.model(patch.to(device), encode=True)
            features.append(feature.cpu())
            
            flipped_feature = extractor.model(torch.flip(patch, (3, )).to(device), encode=True)
        ...
        # Initial predictions
        bbox = bboxes[:1].clone().to(device)
        bbox_center = bbox[:, :2].clone()
        bbox_scale = bbox[:, 2].clone() / 200
        kwargs = {'img_w': torch.tensor(res_w).repeat(1).float().to(device),
                    'img_h': torch.tensor(res_h).repeat(1).float().to(device),
                    'bbox_center': bbox_center, 'bbox_scale': bbox_scale}

        pred_global_orient, pred_pose, pred_shape, _ = extractor.model(init_patch.to(device), **kwargs)
        ...
        pred_global_orient, pred_pose, pred_shape, _ = extractor.model(torch.flip(init_patch, (3, )).to(device), **kwargs)
        pred_output = smpl['neutral'].get_output(global_orient=pred_global_orient.cpu(),
                                                body_pose=pred_pose.cpu(),
                                                betas=pred_shape.cpu(),
                                                pose2rot=False)
