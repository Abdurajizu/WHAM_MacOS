...
download_file () {
  ...
  if command -v wget >/dev/null 2>&1; then
    if [[ -n "$post_data" ]]; then
      wget --post-data "$post_data" "$url" -O "$output" --no-check-certificate --continue
    else
      wget "$url" -O "$output" --no-check-certificate --continue
    fi
    return 0
  fi
  ...
  if command -v curl >/dev/null 2>&1; then
    if [[ -n "$post_data" ]]; then
      curl -L --retry 3 --data "$post_data" "$url" -o "$output"
    else
      curl -L --retry 3 "$url" -o "$output"
    fi
    return 0
  fi
...
if ! command -v gdown >/dev/null 2>&1; then
  echo "[ERROR] gdown is required. Install with: pip install gdown==4.6.0" >&2
  exit 1
fi
...
download_file \
  ...
  './dataset/body_models/smplify.zip' \
...
download_file \
  ...
  './dataset/body_models/smpl.zip' \
  "username=$username&password=$password"
unzip -o dataset/body_models/smpl.zip -d dataset/body_models/smpl
...
download_file "https://drive.google.com/uc?id=1pbmzRbWGgae6noDIyQOnohzaVnX_csUZ&export=download&confirm=t" 'dataset/body_models.tar.gz'
