...
from configs.config import get_cfg_defaults, resolve_device, resolve_device
...
def prepare_cfg(device='auto'):
    cfg = get_cfg_defaults()
    cfg.merge_from_file('configs/yamls/demo.yaml')
    cfg.DEVICE = resolve_device(device if device is not None else cfg.DEVICE)
    return cfg
...
class WHAM_API(object):
    def __init__(self, device='auto', detector_backend='auto'):
        self.cfg = prepare_cfg(device=device)
        self.network = build_network(self.cfg, build_body_model(self.cfg.DEVICE, self.cfg.TRAIN.BATCH_SIZE * self.cfg.DATASET.SEQLEN))
        self.network.eval()
        self.detector = DetectionModel(self.cfg.DEVICE.lower(), backend=detector_backend)
    ...
    def wham_inference(self, tracking_results, slam_results, width, height, fps, output_dir):
        ...
        for i in range(len(dataset)):
            batch = dataset.load_data(i, False)
            if batch is None:
                continue
