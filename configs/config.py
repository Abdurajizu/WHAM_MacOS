...
def get_cfg_defaults():
    ...
    out = cfg.clone()
    out.DEVICE = resolve_device(out.DEVICE)
    return out


def resolve_device(device='auto'):
    """Resolve a runtime device with CPU default for non-CUDA environments."""
    ...
    if wanted == 'mps' and not (hasattr(torch.backends, 'mps') and torch.backends.mps.is_available()):
        return 'cpu'
